 ...
 *
 * complete.cpp: Pintool used to calculate the overhead of the funcionality
...
bool isCallValid(const CONTEXT *ctxt, ADDRINT addr, string dump) {
	...
	if (isDirectCall(dump)) { // Direct CALLs
		...
		bool lastCallMatch = lastCall == addr;
		/**
		 * Duplicated code end.
	 	*/	
...
VOID doRET(const CONTEXT *ctxt, ADDRINT returnAddr) {
	...
	stream2 << hex << previousBytes;
	string byteString2(stream2.str());
	byteString2.insert(byteString2.begin(), 14 - byteString2.length(), '0');
	byteString2 = reverseByteOrder(byteString2);
	...
	for (int size = 2; size <= 7; size++) {
		for (int opcode = opE8; opcode <= opFF; opcode++) {
			if (sizeToOpcodes[size][opcode] == 1) {
				...
				if (byteString2.substr(index, 2) == opcodeStr) {
					foundCallOpcode = string(opcodeStr);
					
					if (foundCallOpcode == "ff") {
						precededByCall = \
							checkFFCallSize(byteString2.substr(index+2), size); 
					} else {
						precededByCall = true;
						break;
					}
		...
		if (precededByCall) {
			callDump = string(byteString2.substr(index));
			break;
		}
	}
	/**
