 ...
 *
 * count_endbr64.cpp: Pintool used to count the indirect branch targets that
 * are the endbr64 (0xF30F1EFA) instruction.
 */
...
KNOB<string> outFileKnob(KNOB_MODE_WRITEONCE, "pintool", "o", \
	"count_endbr64.out", "Output file name.");
...
VOID doIndirectBranchOrCall(ADDRINT target) {
	...
	PIN_SafeCopy(&targetBytes, (ADDRINT*) target, 4);

	if (memcmp(&ENDBR64, &targetBytes, 4) == 0) {
